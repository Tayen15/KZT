...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('The name of the command to get more information about the command.')),
    name: "help",
    ...
    async execute(interaction, args) {
        ...
        if (!commandArg) {
            const infoCommands = client.commands.filter(x => x.category === 'info').map((x) => `</${x.name}:${x.id}>`).join(', ');
            const musicCommands = client.commands.filter(x => x.category === 'music').map((x) => `</${x.name}:${x.id}>`).join(', ');
            const minecraftCommands = client.commands.filter(x => x.category === 'minecraft').map((x) => `</${x.name}:${x.id}>`).join(', ');
                ...
                embeds: [
                    {
                        ...
                        fields: [
                            ...
                            { name: 'Minecraft', value: minecraftCommands, inline: false }
                        ],
                        footer: {
